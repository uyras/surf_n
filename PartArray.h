...
public:
     ...
     * @param z
     */
	void calcInteraction(int x, int y, int z);
     ...
     * @return энергия
     */
	double calcEnergy1(int x, int y, int z);
...
//	std::vector<double> processStepX(); //последовательный перебор каждой неравновесной частицы. перебор по строкам
//	std::vector<double> processStepZ(); //последовательный перебор каждой неравновесной частицы. перебор по столбцам
...
private:
	std::vector < std::vector < std::vector < Part > > > parts;
};
